...
impl State {
    async fn new(window: Window) -> Self {
        ...
        let mut fragment_uniform = FragmentUniform{
            resolution: [window.inner_size().width as f32, window.inner_size().height as f32],
            frame: 0.0,
            time: 25.0,
        };

        let fragment_buffer = device.create_buffer_init(
    ...
    pub fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            ...
            self.fragment_uniform.resolution = [new_size.width as f32, new_size.height as f32];
        }
    }
    ...
    fn update(&mut self) {
        self.fragment_uniform.time = self.fragment_uniform.time + 0.01;
        self.queue.write_buffer(&self.fragment_buffer, 0, bytemuck::cast_slice(&[self.fragment_uniform]));
    }
